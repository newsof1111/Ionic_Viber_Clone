(function() {
  ...
  angular.module('ionic.contrib.NativeDrawer', ['ionic'])
    ...
    .controller('drawerCtrl', ['$rootScope', '$scope', '$element', '$attrs', '$ionicGesture', '$ionicBody', '$document','$ionicBackdrop', function($rootScope, $scope, $element, $attr, $ionicGesture, $ionicBody, $document,$ionicBackdrop) {
     ...
     var startDrag = function(e) {
      ...
      var doEndDrag = function(e) {
        ...
        if(isOpen && newX < (-width / 3)) 
        {
           ionic.requestAnimationFrame(function () 
                {                
                      el.style[ionic.CSS.TRANSFORM] = 'translate3d( ' +  (-width - 5)  + 'px,0, 0)';
        ...
        else
         {
           ionic.requestAnimationFrame(function () 
                {                
                      el.style[ionic.CSS.TRANSFORM] = 'translate3d(0px, 0, 0)';
      ...
      var doEndContentDrag = function(e) {
        if (startX > lastX){
          ...
          enableAnimation();
           ionic.requestAnimationFrame(function () 
                {                
                      el.style[ionic.CSS.TRANSFORM] = 'translate3d(-101%, 0, 0)';
        ...
        if(!dragging) {
          ...
          if (isContentDrag && lastX < startX) {
            var drawerOffsetX = lastX - drawerWidth;
             ionic.requestAnimationFrame(function () 
                {                
                      el.style[ionic.CSS.TRANSFORM] =  'translate3d(' + -absX + 'px, 0, 0)';
      ...
      var dragGestureLeft = $ionicGesture.on('dragleft', dragFunction, $document);
      var dragGestureRight = $ionicGesture.on('dragright', dragFunction, $document);
      ...
      var contentDragGestureLeft = $ionicGesture.on('dragleft', onContentDrag, mainContent);
      var contentDragGestureRight = $ionicGesture.on('dragRight', onContentDrag, mainContent);
      var contentDragEndGesture = $ionicGesture.on('dragend', contentDragEndFunction, mainContent);

      $scope.$on('$destroy', function() {

        $ionicGesture.off(dragGestureLeft, 'dragleft', dragFunction);
        $ionicGesture.off(dragGestureRight, 'dragright', dragFunction);
        $ionicGesture.off(dragEndGesture, 'dragend', dragEndFunction);
        $ionicGesture.off(contentTapGesture, 'tap', onContentTap);
        $ionicGesture.off(contentDragGestureLeft, 'dragleft', onContentDrag);
        $ionicGesture.off(contentDragGestureRight, 'dragleft', onContentDrag);
